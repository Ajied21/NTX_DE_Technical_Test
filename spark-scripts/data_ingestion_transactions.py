...
# Define the schema for the transactions dataset
...
transactions_dataset = spark.read.csv("/data/transactions.csv", header=True, schema=transactions_schema)

# Write the dataset to PostgreSQL database
try:
    # Overwrite the 'transactions' table in PostgreSQL with the new data
    transactions_dataset.write.mode("overwrite").jdbc(jdbc_url, 'public.transactions', properties=jdbc_properties)
...
# Read the 'transactions' table from PostgreSQL into a Spark DataFrame
transactions_df = spark.read.jdbc(jdbc_url, 'public.transactions', properties=jdbc_properties)

# Display the first 10 rows of the transactions dataset
